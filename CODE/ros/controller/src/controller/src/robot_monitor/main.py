...
class WebSender ():
    ...
    def __init__(self):
        ...
        self.last_controller = raw_controller()
        self.last_controller.RX = 0
        self.last_controller.RY = 0
        self.last_controller.LY = 0
        self.last_controller.LX = 0
    ...
    def update_us(self, data):
        with self.us_lock:
            self.last_us.measurements = data.measurements

    def getValue(self):
        with self.us_lock and self.controller_lock:
            return {'controller' : {'RX' : self.last_controller.RX,
                                    'RY' : self.last_controller.RY,
                                    'LX' : self.last_controller.LX,
                                    'LY' : self.last_controller.LY},
